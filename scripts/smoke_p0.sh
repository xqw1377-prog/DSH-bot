...
python scripts/run_crypto_bot.py --once
COUNT3=$(curl -sf "$QUANT_GATEWAY_URL/v1/approvals" | python -c "import json,sys; print(len(json.load(sys.stdin)))")
if [[ "$COUNT3" != "1" ]]; then
  echo "[smoke] memory not recovered; approvals=$COUNT3" >&2
  exit 1
fi
