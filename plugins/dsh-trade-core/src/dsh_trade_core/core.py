"""共享交易执行核心（TradeExecutionCore）。

从 Crypto Bot 提取的单版本执行闭环：信号 → 预览 → 人工审批（任务持久化、
记忆去重）→ 执行前三重再校验（审批绑定/信号时效/风险边界）→ 提交 →
成交生命周期 → 数值守恒对账 → DONE。

市场差异（交易时段、整手、涨跌停、T+1 等）全部通过 MarketPolicy 注入，
任何 Bot 不得复制本流程（防止审批、崩溃恢复、对账逻辑分叉）。
"""
...
class TradeExecutionCore:
    ...
    def __init__(
        ...
        from dsh_trade_core.policy import MarketPolicy
    ...
    def _session_blocked(self) -> str | None:
        """交易时段检查：市场闭市/午休/周末时跳过新信号（在途任务照常恢复）。"""
        return self.policy.session_blocked()

    def _default_quantity(self, signal: dict) -> str:
        return self.policy.default_quantity(signal)

    def _validate_order(self, quantity: str, est_price) -> str | None:
        return self.policy.validate_order(self.market, quantity, est_price)
        ...
        if status == "APPROVED":
            if task["status"] == "AWAITING_APPROVAL":
                ...
                if problem is not None:
                    ...
                    session.events.emit(
                        "order/unknown", self._market_label, "bot", self.name,
            ...
            session.events.emit(
                "approval/rejected", self._market_label, "bot", self.name,
    ...
    def _handle_approval_missing(self, session: BotSession, task: dict) -> None:
        ...
        session.events.emit(
            "incident/opened", self._market_label, "bot", self.name,
            {"approval_id": approval_id, "task_id": task["task_id"],
    ...
    def _revalidate(self, session: BotSession, task: dict,
        ...
        try:
            fresh = self.gateway.preview_order(OrderIntent(
                ...
                quantity=signal.get("quantity") or self._default_quantity(signal),
        ...
        try:
            delta = (Decimal(str(fresh_risk.get("position_after", "0")))
                     - Decimal(str(fresh_risk.get("position_before", "0"))))
            if delta > Decimal(boundary.get("max_quantity", "0")):
                return (f"position delta {delta} exceeds approved "
                        f"max_quantity {boundary.get('max_quantity')}")
        except Exception:
    ...
    def _dispatch_submit(self, session: BotSession, task: dict,
        ...
        session.events.emit(
            "order/submitted", self._market_label, "bot", self.name,
            {
    ...
    def _reconcile_fill(self, session: BotSession, task: dict) -> None:
        ...
        if order_status == "CANCELLED":
            ...
            session.events.emit(
                "order/cancelled", self._market_label, "bot", self.name,
        ...
        if order_status == "PARTIALLY_FILLED":
            ...
            session.events.emit(
                "order/filled", self._market_label, "bot", self.name,
                {"task_id": task["task_id"], "order_id": order_id,
                 "market": self._market_label,
    ...
    def _reconcile_account(self, session: BotSession, task: dict) -> None:
        ...
        if match is None or Decimal(str(match["quantity"])) < expected:
            ...
            session.events.emit(
                "account/mismatch", self._market_label, "bot", self.name,
                {"task_id": task["task_id"], "order_id": order_id,
        ...
        if filled_qty < expected:
            ...
            session.events.emit(
                "account/mismatch", self._market_label, "bot", self.name, reconciliation)
            return
        session.events.emit(
            "account/reconciled", self._market_label, "bot", self.name, reconciliation,
        )
    ...
    def _process_new_signals(self, session: BotSession) -> None:
        blocked = self._session_blocked()
        if blocked:
            session.memory.remember(
                f"{self._market_label} 当前不可交易：{blocked}，跳过新信号",
                kind="session-blocked", tags=["session-blocked"],
            )
            return
    ...
    def _preview(self, session: BotSession, task_id: str, signal: dict) -> dict | None:
        ...
        # 市场规则校验（整手/涨跌停等）：预览成功但违反市场规则同样失败关闭
        est_cost = preview.get("estimated_cost")
        qty = Decimal(str(intent.quantity))
        est_price = None
        if est_cost is not None and qty > 0:
            est_price = Decimal(str(est_cost)) / qty
        rule_problem = self.policy.validate_order(
            self.market, str(intent.quantity), est_price, signal["symbol"])
        if rule_problem is not None:
            session.tasks.transition(task_id, "PRE_SUBMIT_FAILED")
            session.memory.remember(
                f"市场规则拦截（{signal['signal_id']}）: {rule_problem}",
                kind="market-rule-blocked",
                tags=[f"signal:{signal['signal_id']}", "market-rule"],
            )
            return None
        ...
        payload.update(
            ...
            risk_boundary={
                ...
                "position_before": str(risk.get("position_before", "0")),
    ...
    def _request_approval(self, session: BotSession, task_id: str,
        ...
        _get().commit()
        session.events.emit(
            "approval/requested", self._market_label, "bot", self.name,
            {
