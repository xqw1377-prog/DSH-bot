...
def _keys() -> set[str]:
    raw = os.environ.get("RISK_POLICY_API_KEYS") or ""
...
def require_service_key(
    ...
    if not keys:
        raise HTTPException(
            status_code=503,
            detail="risk-policy fail-closed: RISK_POLICY_API_KEYS required",
        )
    if not x_api_key or x_api_key not in keys:
        raise HTTPException(
            status_code=401,
            detail="missing or invalid risk-policy service identity",
        )
