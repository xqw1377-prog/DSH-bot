...
def _keys() -> set[str]:
    raw = os.environ.get("RISK_AUDITOR_API_KEYS") or ""
...
def require_service_key(
    ...
    if not keys:
        raise HTTPException(
            status_code=503,
            detail="risk-auditor fail-closed: RISK_AUDITOR_API_KEYS required",
        )
    if not x_api_key or x_api_key not in keys:
        raise HTTPException(
            status_code=401,
            detail="missing or invalid risk-auditor service identity",
        )
